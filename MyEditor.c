...
struct editorConfig {
    int screenrows;
    int screencols;
    int cx, cy; //index in chars
    int rx;     //index in render
    int numrows;
    int rowoff; //row offset
    int coloff;
    int dirty;  //whether be modified
    erow* row;   //edit row
    char* filename;
    char statusmsg[80];
...
void enableRawMode() {
  ...
  //set CS to 8 bits/byte
  raw.c_cflag |= (CS8);
  raw.c_cc[VMIN] = 0;// min input number of bytes
  raw.c_cc[VTIME] = 1;//max time wait before read()
    ...
    while(i < row->rsize) {
        ...
        if(prev_sep) {
            ...
            for(j = 0; keywords[j]; j++) {
                ...
                if(!strncmp(&row->render[i], keywords[j], klen) && isSepatator(row->render[i + klen])) {
...
void editorSelectSyntaxHighlight() {
    ...
    for(unsigned int j = 0; j < HLDB_ENTRIES; j++) {
        ...
        while(s->filematch[i]) {
            ...
            if((is_ext && !strcmp(ext, s->filematch[i])) || (!is_ext && strstr(E.filename, s->filematch[i]))) {
                E.syntax = s;
...
void editorSave() {
    ...
    free(buf);
    editorSetStatusMessage("Save error! I/O error: %s", strerror(errno));
}
...
void editorFind() {
    ...
    char* query = editorPrompt("Search: %s (Press ESC to cancel)", editorFindCallback);
    if(query) {
        free(query);
    }
    else {
    ...
    for(i = 0; i < E.screenrows; i++) {
        ...
        if(filerow >= E.numrows){
            if(E.numrows == 0 && i == E.screenrows / 3) {
                char welcome[80];
                int welcome_len = snprintf(welcome, sizeof(welcome), "My Editor -- Version %s", VERSION);
        ...
        abAppend(ab, "\x1b[K", 3);  //clean a line
        abAppend(ab, "\r\n", 2);
    }
}
...
void editorDrawStatusBar(struct abuf* ab) {
    ...
    char status[80];
    char rstatus[80];
    int len = snprintf(status, sizeof(status), "%.20s %s", E.filename ? E.filename : "[No Name]", E.dirty ? "Modified" : "");
...
void editorRefreshScreen() {
    ...
    abAppend(&ab, "\x1b[H", 3);  //let cursor top
    ...
    write(STDOUT_FILENO, ab.b, ab.len); //a "big" write
    abFree(&ab);
}
